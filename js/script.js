...
smallestAlignment('.footer_support-info .footer_heading p', 'width', [
  '.footer_support-info .footer_additionaly',
])
...
function historycalCalendar() {
  ...
  function navigationCalendar(data, activeClass) {
    let prev = document.querySelector(
      '.history_calendar-navigation .arrow:first-child'
    )
    let next = document.querySelector(
      '.history_calendar-navigation .arrow:last-child'
    )
...
function slider() {
  ...
  if (box) {
    $('.portfolio_slider > div').owlCarousel({
      ...
      nav: true,
      navText: [
        '<div><div class="arrowBlack"></div></div>',
        '<div><div class="arrowBlack"></div></div>',
      ],
    })
  }
...
function addingCardsToDiv() {
  ...
  let allCards = document.querySelectorAll(
    '.addInContainer .generalPlanning_container-item'
  )
...
function hightCartMobileServices() {
  let carts = $(
    '#mobileGeneralPlanning_container .generalPlanning_container-item'
  )
  let allSize = []
